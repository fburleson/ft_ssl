...
pub_key_t parse_pub_key(const char *content)
{
    ...
    pub_key.p = base64_to_dec((const base64_t)(content + cidx));
    cidx += BASE64_N_DIGITS;
    pub_key.q = base64_to_dec((const base64_t)(content + cidx));
    cidx += BASE64_N_DIGITS * 2;
...
void print_priv_key(const priv_key_t *const priv_key, const int fd)
{
    ...
    print_base64(key_mod(priv_key->p, priv_key->q), fd);
...
void print_pub_key(const pub_key_t *const pub_key, const int fd)
{
    ...
    print_base64(pub_key->p, fd);
    print_base64(pub_key->q, fd);
    print_base64(key_mod(pub_key->p, pub_key->q), fd);
